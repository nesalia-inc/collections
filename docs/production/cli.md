...
In development, you can use automatic schema syncing via the Next.js plugin:
...
1. **Config Watching**: The plugin watches `src/lib/collections.ts` for changes
2. **Auto-Push**: When collections change, schema is automatically pushed to the database
3. **Development Only**: This only runs in development mode

```typescript
// src/lib/collections.ts - Modify this file
const posts = collection({
  slug: 'posts',
  fields: {
    title: field({ fieldType: f.text() }),
    // Add new field
    content: field({ fieldType: f.text() })  // AUTO-PUSH TRIGGERED
  }
})
```
